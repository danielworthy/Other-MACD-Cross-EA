#include "includes/common_def.mqh"
#include "includes/common1.mqh"
#include "includes/order_functions.mqh"
#include "includes/init.mqh"
#include "includes/setup.mqh"
#include "includes/order_info.mqh"
#include "includes/trailing_stop_functions.mqh"
...
const double      cSTOPLOSS_MULT = 3;
const double      cTRAILINGSTOP_MULT = 2.5;
...
double gUPF[][2];
double gDNF[][2];
double gATR, gCLOSE1, gOPEN1, gHIGHEST, gLOWEST =0;
...
//   if (Time0 == Time[0]) return; Time0 = Time[0];
  ...
  subORDERSTATUS();
   ...
   subFRACTALS_ARRAY(4);
   if (gUPF1 <= 0 || gUPF2 <= 0 || gDNF1 <= 0 || gDNF2 <= 0) {
      subDISPLAYADD("Fractals", "Insufficient fractal history");
...
// get values before comparisons!!   
   ...
   gHIGHEST = High[iHighest(NULL,0,MODE_HIGH,fBARCOUNT,1)];
   gLOWEST = Low[iLowest(NULL,0,MODE_LOW,fBARCOUNT,1)];
   ...
   if (!cATR_STOPS) { subStops(500, (gFR_AV_RANGE2*cSTOPLOSS_MULT)/gPOINT_VALUE, (gFR_AV_RANGE2*cTRAILINGSTOP_MULT)/gPOINT_VALUE); }  
      else if (cATR_STOPS) 
         { subStops(500, (gATR*cSTOPLOSS_MULT)/gPOINT_VALUE, (gATR*cTRAILINGSTOP_MULT)/gPOINT_VALUE); }
   ...
   if (fBUY_STOPLOSS  < gATR*cSTOPLOSS_MULT ) fBUY_STOPLOSS  = gATR*cSTOPLOSS_MULT;  
   if (fSELL_STOPLOSS < gATR*cSTOPLOSS_MULT ) fSELL_STOPLOSS = gATR*cSTOPLOSS_MULT;  
...
// current tf - fractal shape - break into own code
   ...
   if ( TRADE  ) {   
      if ( gBUYTRADES == 0  && gBUY_PENDING == 0  && fCONDITION == BUY  && Bid > gDNF1 && Ask < gUPF1  )  {  subStops(500, fBUY_STOPLOSS/gPOINT_VALUE,  (gATR*cTRAILINGSTOP_MULT)/gPOINT_VALUE);  subSETORDERS(BUY, gUPF1); }
      if ( gSELLTRADES == 0 && gSELL_PENDING == 0 && fCONDITION == SELL && Ask < gUPF1 && Bid > gDNF1  )  {  subStops(500, fSELL_STOPLOSS/gPOINT_VALUE, (gATR*cTRAILINGSTOP_MULT)/gPOINT_VALUE);  subSETORDERS(SELL, gDNF1); }
       }
...
if ( ! subISTRAILINGSTOPSET() ) {
   ...
   if (MarginCheck() < cMARGIN && AccountProfit() > 1  ) { subDeleteOrders(ALL); subCLOSE(ALL); }
