...
void subDraw()
{
   ...
   //ObjectCreate("Rectangle", OBJ_RECTANGLE, 0, iTime(NULL,0,1), gHIGHEST, iTime(NULL,0,bc), gLOWEST);
   //ObjectSet("Rectangle", OBJPROP_COLOR, boxcolor); 

 
   subDRAWLINE("High", gHIGHEST, Orange, 2, 0);
   subDRAWLINE("Low", gLOWEST, Orange, 2, 0);
 
   subDRAWLINE("DHigh", iHigh(NULL,PERIOD_D1,1), Blue, 1, 0);
   subDRAWLINE("DLow",  iLow(NULL,PERIOD_D1,1), Blue, 1, 0);
...
//linetext = "dgHIGHl"; ObjectDelete(linetext);  ObjectCreate(linetext,OBJ_HLINE,0,0,gHIGHEST); ObjectSet(linetext, OBJPROP_COLOR, Yellow); ObjectSet(linetext, OBJPROP_WIDTH, 1); ObjectSet(linetext, OBJPROP_STYLE, 0);
...
// 0 means autoset
   ...
   if ( cATR_STOPS ) gSTOPLOSS =  iATR(NULL,0,100,1) /gPOINT_VALUE  * cSTOPLOSS_MULT  ;
   if ( cATR_STOPS ) gTRAILINGSTOP = iATR(NULL,0,100,1) / gPOINT_VALUE * cTRAILINGSTOP_MULT;
