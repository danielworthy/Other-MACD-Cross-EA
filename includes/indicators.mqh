...
double subAV_FRACTAL_RANGE(int fRANGE)
  {
   ...
   int counter=0;
   int found_up_fractals=0;
   int found_dn_fractals=0;
...
// find x up fractals
   ...
   while(found_up_fractals < fRANGE && counter < Bars)
     {
      fUPF = iFractals(NULL,0,MODE_UPPER,counter);
      if(fUPF > 0)
        {
         fUPF_ARRAY[found_up_fractals] = fUPF;
         found_up_fractals++;
        }
      counter++;
     }
...
// reset for finding down fractals
   ...
   while(found_dn_fractals < fRANGE && counter < Bars)
     {
      fDNF = iFractals(NULL,0,MODE_LOWER,counter);
      if(fDNF > 0)
        {
         fDNF_ARRAY[found_dn_fractals] = fDNF;
         found_dn_fractals++;
        }
      counter++;
     }
...
// we've found x up & dn fractals. Now we need the range of each and average them
   ...
   int fPAIR_COUNT = MathMin(found_up_fractals, found_dn_fractals);
   if(fPAIR_COUNT <= 0)
      return(0);
   ...
   for(counter=0; counter<fPAIR_COUNT; counter++)
     {
   ...
   fAV_RANGE = fTOTAL_RANGE / fPAIR_COUNT;
...
// we've found x up & dn fractals. Now we need the price of each and average them
   ...
   fUPF_PRICE = fUPF_PRICE / fPAIR_COUNT;
   fDNF_PRICE = fDNF_PRICE / fPAIR_COUNT;
   fAVF_PRICE = fAVF_PRICE / fPAIR_COUNT;
   ...
   subDISPLAYADD("Bars for Av. Price", fPAIR_COUNT);
   ...
   for(for_counter=0; for_counter<fNUMBER_OF_FRACTALS; for_counter++)
     {
      while(while_counter < Bars)
        {
         double fUpperFractal = iFractals(NULL,0,MODE_UPPER,while_counter);
         if(fUpperFractal > 0)
           {
            gUPF[for_counter][0] = fUpperFractal;
   ...
   for(for_counter=0; for_counter<fNUMBER_OF_FRACTALS; for_counter++)
     {
      while(while_counter < Bars)
        {
         double fLowerFractal = iFractals(NULL,0,MODE_LOWER,while_counter);
         if(fLowerFractal > 0)
           {
            gDNF[for_counter][0] = fLowerFractal;
