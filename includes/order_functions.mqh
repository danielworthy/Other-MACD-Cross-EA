...
void subDeleteOrders(string fORDERTYPE)
{
      ...
      for(int counter=OrdersTotal()-1;counter>=0;counter--)
   {
      ...
      if (OrderSymbol() == Symbol() ) {
         ...
         if( fORDERTYPE == ALL &&  (OrderType() >= OP_BUYLIMIT && OrderType() <= OP_SELLSTOP)  ) x = OrderDelete(OrderTicket(),Green);
   ...
   for(int counter=OrdersTotal()-1;counter>=0;counter--)
      {
         ...
         if((fORDERTYPE == ALL || fORDERTYPE == EVERYTHING) && OrderSymbol()==Symbol() && (OrderType() == OP_BUY || OrderType() == OP_SELL) )  ticket=OrderClose(OrderTicket(),OrderLots(),OrderClosePrice(),10,Violet);
